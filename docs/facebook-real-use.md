...
- Publish a Facebook Page text post only through the guarded Publish Queue action after all safety gates pass.

## What Is Still Disabled

- Instagram, Threads, YouTube, TikTok, LinkedIn, and X real posting.
- Facebook image/video publishing.
...
The guarded Facebook text-post path also needs:
...
For guarded Facebook text-post testing, you must intentionally change both publishing flags:
...
Do not enable these until you have read this guide, connected a test Facebook Page, confirmed emergency pause is off, and confirmed you are ready for the app to create a real Page text post.
...
The current guarded Facebook text-post service requires this development-only token mode because OS keychain/encrypted token storage is not implemented yet. That is acceptable for personal local testing only, not production.
...
## Publishing A Facebook Text Post
...
9. You are willing to create a real Facebook Page post.
...
After you approve and schedule a Facebook draft, use **Manual Export** from Publish Queue. That creates a local posting package with caption, hashtags, media references, and instructions.
