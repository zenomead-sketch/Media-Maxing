...
Publish queue rows are local readiness records only. Queue statuses such as `ready`, `mock_published`, and `manually_exported` must never be presented as real platform publishing.
