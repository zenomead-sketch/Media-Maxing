...
`platform_published` means a guarded real platform action completed and wrote an audit trail. In this build, that state is only expected from the guarded Facebook Page text or single-image post path.
