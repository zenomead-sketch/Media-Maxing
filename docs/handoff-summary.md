...
The current handoff is `0.1.0-local-test`. It is ready for local testing, not production SaaS. Real publishing is locked by default; guarded Facebook Page text or single-image posting exists for personal local testing only.
...
- Real publishing is locked by default, except guarded Facebook Page text or single-image posting after explicit setup.
